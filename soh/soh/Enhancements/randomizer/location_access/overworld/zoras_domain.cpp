...
void RegionTable_Init_ZorasDomain() {
    ...
    areaTable[RR_ZORAS_DOMAIN] = Region("Zoras Domain", "Zoras Domain", {RA_ZORAS_DOMAIN}, NO_DAY_NIGHT_CYCLE, {
        ...
        EventAccess(&logic->KingZoraThawed,   []{return logic->KingZoraThawed    || (logic->IsAdult && logic->BlueFire());}, true),
        EventAccess(&logic->DeliverLetter,    []{return logic->DeliverLetter     || (logic->CanUse(RG_RUTOS_LETTER) && logic->IsChild && ctx->GetOption(RSK_ZORAS_FOUNTAIN).IsNot(RO_ZF_OPEN));}, true),
    }, {
        //Locations
        ...
        LOCATION_NNL(RC_ZD_KING_ZORA_THAWED,                logic->KingZoraThawed),
        LOCATION_NNL(RC_ZD_TRADE_PRESCRIPTION,              logic->KingZoraThawed && logic->CanUse(RG_PRESCRIPTION)),
