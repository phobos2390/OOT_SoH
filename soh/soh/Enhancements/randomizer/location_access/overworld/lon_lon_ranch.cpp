...
void RegionTable_Init_LonLonRanch() {
    ...
    areaTable[RR_LON_LON_RANCH] = Region("Lon Lon Ranch", "Lon Lon Ranch", {RA_LON_LON_RANCH}, NO_DAY_NIGHT_CYCLE, {
        ...
        EventAccess(&logic->LinksCow,   []{return logic->LinksCow || (logic->HasItem(RG_CHILD_WALLET) && logic->CanUse(RG_EPONAS_SONG) && logic->IsAdult && logic->AtDay);}, true),
    }, {
        //Locations
        LOCATION_NNL(RC_SONG_FROM_MALON,     logic->IsChild && logic->HasItem(RG_ZELDAS_LETTER) && logic->HasItem(RG_FAIRY_OCARINA) && logic->AtDay),
        LOCATION_NNL(RC_LLR_GS_TREE,         logic->IsChild && logic->CanBonkTrees()),
        ...
        LOCATION_NNL(RC_LLR_TREE,            logic->IsChild && logic->CanBonkTrees()),
    }, {
        //Exits
